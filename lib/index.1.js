...
document.addEventListener('DOMContentLoaded', () => {
  ...
  audio.src = './assets/audio/slowmotion.mp3';
  audio.muted = "muted";
  ...
  if (canvas.getContext) {
    ...
    startButton.on('click', () => {
      audio.play().then( () => console.log('hello'))
      debugger;
    ...
    function drawCoins() {
      for (let i = 0; i < coins.length; i++) {
        ...
        if (coin.collected) {
          coin.collect();
          const stagger = coin.index % 2 ? coin.index % 4 * 1.7 : coin.index % 4 * -1.5;
          coin.x = x2 + stagger;
          coin.y = y2 + stagger * .8;
          ctx.drawImage(coin.image, coin.x, coin.y, 40, 40)
          continue;
        }
